...
3. **既存チケットの編集か**を判断する：引数が既存のチケットを指している（`docs/tickets/<正規化した id>.md` が存在する）場合は、新規作成ではない。内容を示し、編集するか `/flow dev <ticket-id>` に進むかを尋ねる。編集する場合は手順 5・6 の要領で合意した内容だけを書き換え、`github` なら issue の本文を同期する（`references/github.md`）。**既存ファイルを黙って上書きしない。**
...
Issue: #<番号>
<!-- tracker が github のときだけ書く。local のときはこの行ごと書かない。`Issue:` は英語の固定キー。 -->

## 背景
<!-- なぜ必要か。解決したい問題や動機、誰が影響を受けるか。 -->

## 要件
<!-- 何をするか。1 項目 1 行の箇条書きで、実装方法ではなく振る舞いを書く。github のときは issue にそのまま載る。 -->

## 受け入れ条件
<!-- 何を満たせば完了か。各項目を「- [ ]」で、できた／できていないで判定できる形で書く。github のときは issue にそのまま載る。 -->
