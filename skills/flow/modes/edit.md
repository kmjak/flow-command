...
3. GitHub 連携なら、issue の本文がチケットから生成したものと一致するか確かめる（`references/github.md` の「本文の同期」と同じ比較）。違っていれば、誰かが GitHub 上で本文を編集したということなので、差分を示し、**同期すると上書きされて消える**ことを伝えてから続けるか尋ねる（issue は一方通行の写しなので、issue の内容をチケットに取り込むことはしない。残したい内容があれば、この編集でチケットに書く）。
...
7. **進行中の run がある場合**（`docs/flow/<ticket-id>/main.md` があり、`Status` が `done`・`canceled` 以外）：変更がどのフェーズの成果に影響するかを示す（例：受け入れ条件が変わった → Approach の方針と Plan の commit 分割を見直す必要がある。背景の言い回しだけ → 影響なし）。そのうえで AskUserQuestion で戻すフェーズを選んでもらう。選択肢は、今のフェーズ以前の Research・Approach・Plan から影響に応じて推奨を付けたものと、「戻さない」。
   - 戻す場合は `Status` を `<選んだフェーズ>:in-progress` に、`Updated` を今の時刻にする。セクションは消さず、戻したフェーズのセクションの先頭に `> チケット編集（<YYYY-MM-DD HH:MM>）：<変更点の要約>` の 1 行を足す（再開したときに、なぜ戻ったのかが分かるように）。後のフェーズのセクションは、そのフェーズをやり直すときに書き直す。
   - ブランチや commit には触らない。run を丸ごと捨ててやり直したい場合は `/flow reset` を案内する。
8. 行ったこと（GitHub 連携なら issue の URL も）を伝え、run があれば `/flow dev <ticket-id>` で戻したフェーズから再開できることを案内する。
